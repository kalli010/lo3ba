...
int draw_wall_slice(t_mlx *mlx, float distance, int i, int wall_color)
{
  ...
  wall_h = SCREEN_HEIGHT / distance;
  top = (SCREEN_HEIGHT / 2) - (int)(wall_h / 2);
  bottom = (SCREEN_HEIGHT / 2) + (int)(wall_h / 2);
  y = -1;
  while (++y < top)
    mlx_pixel_put(mlx->mlx, mlx->win, i, y, 0x4ff2f7);
  while (top <= bottom)
  {
    mlx_pixel_put(mlx->mlx, mlx->win, i, top, wall_color);
    top++;
  }
  while (++top <= SCREEN_HEIGHT)
    mlx_pixel_put(mlx->mlx, mlx->win, i, top, 0xf0a64d);
  return (0);
}

int init_ray_data(t_draw_map *d, t_data *data)
{
  d->p_x = data->player->x + 0.5;
  d->p_y = data->player->y + 0.5;
...
void draw_map(t_mlx *mlx, t_data *data)
{
    ...
    else
  {
    ...
    draw_wall_slice(mlx, wall_dist, d.i, wall_color);
  }
}
...
int pixel_put(t_mlx *mlx, float x, float y, int color)
{
  ...
  while (++i < mlx->data->pixel_size)
  {
    ...
    while (++j < mlx->data->pixel_size)
    {
      ...
      //   mlx_pixel_put(mlx->mlx, mlx->win,10 + y * mlx->data->pixel_size + j, \
      //                 10 + x * mlx->data->pixel_size + i, 0x32a852);
      // else
      mlx_pixel_put(mlx->mlx, mlx->win, 10 + y * mlx->data->pixel_size + j, \
                    10 + x * mlx->data->pixel_size + i, color);
    }
  }
}
...
int draw_minimap(t_mlx *mlx, t_data *data)
{
  ...
  // if (data->map[(int)mlx->player->y + 1][(int)mlx->player->x + 1] != '1')
...
void draw_rays(t_mlx *mlx, t_data *data)
{
  ...
  while(++d.i < NUM_RAYS)
  {
    ...
    d.ray_x = data->player->x + 0.5;
    d.ray_y = data->player->y + 0.5;
    ...
    while (1)
    {
      ...
      if (d.ray_x < 0 || d.ray_y < 0 || !data->map[(int)d.ray_y] \
        ...
        break;
      // if(data->map[(int)d.ray_y][(int)d.ray_x] != 'P')
      mlx_pixel_put(mlx->mlx, mlx->win,10 + d.ray_x * data->pixel_size, \
                    10 + d.ray_y * data->pixel_size, 0xFF0000);
    }
  }
}
